from django import forms#通过 forms 模块，允许将表单控件与python文件相结合
...
class RegisterForm(forms.Form):
    ...
    )#CharField：获取用户输入的信息 TextInput：允许用户输入信息
   ...
   #以下为信息验证
    def clean_username(self):
        ...
        if len(username) < 6:
            ...
            filter_result = User.objects.filter(username=username)#向数据库中填入数据
